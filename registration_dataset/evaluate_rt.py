...
def evaluate_rt(gt_file_path, predict_file_path):
    ...
    for i in range(1, len(predict_reg)):
        ...
        predict_idx1, predict_idx2, predict_t, predict_rot = reg_result_row_to_array(
            predict_row)
...
def main():
    ...
    dataset_path = 'registration_dataset'
    ground_truth_reg_result_path = os.path.join(
        dataset_path, 'groundtruths.txt')
    ...
    for i in range(1, 5):
        visualize_row_idx = i
        reg_list = read_reg_results(os.path.join(
            dataset_path, 'reg_result.txt'), splitter=',')
        idx1, idx2, t, rot = reg_result_row_to_array(
            reg_list[visualize_row_idx])
        # print("i", i, "idx1", idx1, "idx2", idx2)

        src_np = read_oxford_bin(os.path.join(
            dataset_path, 'point_clouds', '%d.bin' % idx1))[0:3, :]
        dst_np = read_oxford_bin(os.path.join(
            dataset_path, 'point_clouds', '%d.bin' % idx2))[0:3, :]
